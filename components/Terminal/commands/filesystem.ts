...
function normalizePath (state: State, path: string): string {
  ...
  if (path.startsWith("/") && !normalizedPath.startsWith("/")) { normalizedPath = "/" + normalizedPath; }
...
function resolvePath (state: State, path?: string): string {
  let fullPath;
  if (path === undefined) { fullPath = cwd(state, []); } else if (path.startsWith("/")) { fullPath = path; } else { fullPath = cwd(state, []) + "/" + path; }
...
const cd: CommandHandler = (state, params) => {
  ...
  if (!exists(state, path)) { return `Cannot cd to ${path}: directory does not exist`; }
...
const ls: CommandHandler = (state, params) => {
  ...
  if (item === null) { return `Cannot access '${path}': no such file or directory`; }
